...
func FromContext(ctx context.Context) *zap.SugaredLogger {
	...
	if ginContext, ok := ctx.(*gin.Context); ok {
		if request := ginContext.Request; request != nil {
			ctx = request.Context()
		}
	}
...
// From returns the logger stored in the context. If no such logger
// exists, a default logger is returned.
...
// PopulateLogger populates the request context with a random UUID if one
// does not already exist.
func PopulateLogger() gin.HandlerFunc {
	return func(c *gin.Context) {
		baseLogger := DefaultLogger()

		reqID := acquireRequestID(c)
...
func Attach(reqID string) context.Context {
	baseLogger := DefaultLogger()
	newLogger := baseLogger.With("request_id", reqID)
	ctx := context.Background()
	ctx = WithLogger(ctx, newLogger)
	return ctx
}
func acquireRequestID(c *gin.Context) string {
	reqID := c.GetString(string(contextKeyRequestID))
	if reqID == "" {
		if reqID = c.GetHeader(HeaderXRequestID); reqID == "" {
